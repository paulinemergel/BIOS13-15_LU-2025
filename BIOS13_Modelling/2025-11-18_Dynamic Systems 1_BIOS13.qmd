...
i\) Start by setting n = $n_0$, 𝑡 = 0\
ii) Choose a small Δ𝑡\
iii) Calculate 𝑓(𝑡, n)\
iv) Calculate Δ𝑥 = 𝑓(𝑡, n)Δt\
v) Update n ← n + Δ𝑥, 𝑡 ← 𝑡 + Δ𝑡\
...
Re-write the program in exercise 4 to a general ode-solver, taking the function 𝑓(𝑡, 𝑥) as an input parameter. Test it with functions with known solutions, such as exponential or logistic growth. Another possible test is a function 𝑓(𝑡, 𝑥) = cos (𝑡) with solution 𝑦 = sin(𝑡) + 𝑦(0) (can you see why?).
